...
process {
  ...
  function global:au_GetLatest {
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute(
      <#Category#>'PSAvoidLongLines',
      <#ParameterName#>'',
      Justification = 'The SemVer regex cannot easily be shortened.',
      Scope = 'Function',
      Target = '*')]
    param()
    ...
    $contributors = Invoke-RestMethod -Uri $contributorsUrl |
      ForEach-Object -Process {
        $_ |
          Where-Object -FilterScript { $_.type -eq 'User' -and $knownBotUsers -notcontains $_.login } |
          Select-Object -Property login, contributions, url
        }
    $totalContributions = $contributors |
      Select-Object -ExpandProperty contributions |
      Measure-Object -Sum |
    ...
    $contributors |
      ForEach-Object -Process {
        $addMemberArgs = @{
          ...
          Value      = ($_.contributions / $totalContributions) * 100
        }
        $_ | Add-Member @addMemberArgs
      }
    $contributorsOfNote = $contributors |
      Where-Object -FilterScript { $_.PercentageOfContributions -gt 1 } |
      ForEach-Object { Invoke-RestMethod -Uri $_.url } |
    ...
    $remotes = $(
      try {
        git remote -v |
          Select-String -Pattern '^(?<name>[^\s]+)\s+(?<url>[^\s]+)\s+\(fetch\)$' -NoEmphasis |
          Select-Object -Property @{
            ...
            Expression = {
              ...
              Expression = {
                $_.Matches.Groups |
                  Where-Object -FilterScript { $_.Name -eq 'url' } |
                  Select-Object -ExpandProperty Value
                }
              }
      }
    ...
    $releases | ForEach-Object {
      ...
      if ($release.draft -eq 'true') {
        Write-Information -MessageData "Release '$($release.name)' is in draft mode. Skipping this release for distribution."
        return;
      }

      $versionParts = $release.tag_name |
        Select-String -Pattern $versionRegex |
        ...
        [array]$assetUrls = $release.assets |
          Where-Object -FilterScript { $_.name -Match '\.exe$' } |
          ...
          if ($null -eq $checksumFileUrl) {
            throw [System.NullReferenceException]"Using the regex '$checksumRegex', could not find the checksum file among the assets: `n" +
            "$(@($release.assets | Select-Object -ExpandProperty name) -join ",`n")"
          }
          ...
          function Select-NamedRegexGroup {
            [CmdletBinding()]
            param(
              [Parameter()]
              $Pattern,
            ...
            $checksum32 = Select-NamedRegexGroups -Pattern '(?<checksum>[0-9a-f]+)\s+.*-386\.exe$' -Path $tempChecksumFile |
              ...
              Select-Object -ExpandProperty Value
              $checksum64 = Select-NamedRegexGroup -Pattern '(?<checksum>[0-9a-f]+)\s+.*-amd64\.exe$' -Path $tempChecksumFile |
                Where-Object -FilterScript { $_.Name -eq 'checksum' } |
                Select-Object -ExpandProperty Value
